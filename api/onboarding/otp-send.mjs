...
import {createOtpCode,hashOtpCode,normalizePhone,sendTelegramCode,sendWhatsAppCode} from '../_lib/verification.mjs';
...
export default async function handler(req,res){
  ...
  try{
    ...
    const code=channel==='whatsapp'?createOtpCode():null;
    const codeHash=code?hashOtpCode(actor.user.id,phone,code):null;
    const provider=channel==='whatsapp'?await sendWhatsAppCode(phone,code):await sendTelegramCode(phone);
    const [challenge]=await insert('verification_challenges',[{user_id:actor.user.id,channel,phone,provider_request_id:provider.requestId,code_hash:codeHash,status:'pending',expires_at:new Date(Date.now()+10*60000).toISOString()}]);
