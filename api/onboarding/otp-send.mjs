...
import {normalizePhone,sendTelegramCode} from '../_lib/verification.mjs';
...
export default async function handler(req,res){
  ...
  try{
    ...
    const input=await body(req);const phone=normalizePhone(input.phone||actor.profile.phone);const channel=String(input.channel||'telegram');
    ...
    const provider=await sendTelegramCode(phone);
    const [challenge]=await insert('verification_challenges',[{user_id:actor.user.id,channel,phone,provider_request_id:provider.requestId,code_hash:null,status:'pending',expires_at:new Date(Date.now()+10*60000).toISOString()}]);
