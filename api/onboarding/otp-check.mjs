...
export default async function handler(req,res){
  ...
  try{
    ...
    if(new Date(challenge.expires_at).getTime()<Date.now()){await update('verification_challenges',`id=eq.${challenge.id}`,{status:'expired'});throw Object.assign(new Error('Expired'),{status:400,publicMessage:'That code expired. Request a new one.'});}
    if(challenge.attempts>=5)throw Object.assign(new Error('Too many attempts'),{status:429,publicMessage:'Request a new verification code'});
    const valid=challenge.channel==='whatsapp'?await checkWhatsAppCode(challenge.phone,code):await checkTelegramCode(challenge.provider_request_id,code);
    if(!valid){await update('verification_challenges',`id=eq.${challenge.id}`,{attempts:challenge.attempts+1,status:challenge.attempts>=4?'failed':'pending'});throw Object.assign(new Error('Wrong code'),{status:400,publicMessage:'The code is incorrect. Check the message and try again.'});}
    const verifiedAt=new Date().toISOString();
    await update('verification_challenges',`id=eq.${challenge.id}`,{status:'approved',attempts:challenge.attempts+1});
