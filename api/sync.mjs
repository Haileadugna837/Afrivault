  ...
  try{
    ...
    if(domain==='application-status'){
      ...
      if(nextStatus==='approved'){
        ...
        if(!existing.length){const code=`FDRY-${({founder:'F',employee:'E',student:'S',unemployed:'O'})[application.role]||'M'}-${String(new Date().getFullYear()).slice(-2)}${Math.floor(1000+Math.random()*9000)}`;const data=application.data||{};await upsert('profiles',[{id:code,user_id:application.auth_user_id,email:application.email,name:application.name,phone:application.phone||data.phone||'',city:data.city||'',organization:data.company||data.organization||'',title:data.title||'',role:application.role,status:'active',verification:'verified',expires:new Date(Date.now()+365*86400000).toISOString().slice(0,10),notes:'Created from approved membership application.',excluded_benefit_ids:[],preferences:{}}]);
        }
