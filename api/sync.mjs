...
const snakeMember=m=>({id:m.id,email:m.email,name:m.name,phone:m.phone||'',city:m.city||'',organization:m.organization||'',title:m.title||'',role:m.role,status:m.status||'active',verification:m.verification||'pending',expires:m.expires||null,notes:m.notes||'',excluded_benefit_ids:m.excludedBenefits||[],preferences:{language:m.language||'en',twoFactor:Boolean(m.twoFactor),emailAlerts:m.emailAlerts!==false,telegramAlerts:m.telegramAlerts!==false,usageReceipts:m.usageReceipts!==false,preferredOtpChannel:m.preferredOtpChannel||m.phoneVerificationChannel||'whatsapp'}});
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(domain==='application'){
      ...
      const preferred=record.data?.preferredOtpChannel||record.preferredOtpChannel||'whatsapp';
      if(!['whatsapp','telegram'].includes(preferred))throw Object.assign(new Error('Invalid channel'),{status:400,publicMessage:'Choose WhatsApp or Telegram verification'});
      let created;
    ...
    if(domain==='profile'){
      ...
      if(isMember&&(emailChanged||phoneChanged)){record.onboarding_status=emailChanged?'email_pending':'phone_pending';record.phone_verified_at=null;}
      await update('profiles',`user_id=eq.${actor.user.id}`,record);
      if(payload.newPassword||emailChanged)await authAdmin(`/users/${actor.user.id}`,{method:'PUT',body:{...(payload.newPassword?{password:payload.newPassword}:{}),...(emailChanged?{email:payload.email,email_confirm:false}:{})}});
      if(emailChanged){await update('applications',`auth_user_id=eq.${actor.user.id}`,{email:payload.email});const link=await authAdmin('/generate_link',{method:'POST',body:{type:'magiclink',email:payload.email,redirect_to:process.env.APP_URL}});await sendTemplate({to:payload.email,template:'email-verification',payload:{verifyUrl:link.action_link||process.env.APP_URL}}).catch(()=>{});}
    ...
    if(domain==='application-status'){
      ...
      if(nextStatus==='approved'){
        ...
        if(!existing.length){const code=`FDRY-${({founder:'F',employee:'E',student:'S',unemployed:'O'})[application.role]||'M'}-${String(new Date().getFullYear()).slice(-2)}${Math.floor(1000+Math.random()*9000)}`;const data=application.data||{};await upsert('profiles',[{id:code,user_id:application.auth_user_id,email:application.email,name:application.name,phone:application.phone||data.phone||'',city:data.city||'',organization:data.company||data.organization||'',title:data.title||'',role:application.role,status:'active',verification:'pending',onboarding_status:'email_pending',approved_at:new Date().toISOString(),approved_by:actor.user.id,expires:new Date(Date.now()+365*86400000).toISOString().slice(0,10),notes:'Created from approved membership application.',excluded_benefit_ids:[],preferences:{preferredOtpChannel:application.preferred_otp_channel||'whatsapp'}}]);
        }
        const link=await authAdmin('/generate_link',{method:'POST',body:{type:'magiclink',email:application.email,redirect_to:process.env.APP_URL}});await sendTemplate({to:application.email,template:'account-approved',payload:{verifyUrl:link.action_link}});
