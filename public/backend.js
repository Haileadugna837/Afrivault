(function(){
  ...
  const hashParams=new URLSearchParams(location.hash.replace(/^#/,''));const recoverySession=hashParams.get('type')==='recovery'&&hashParams.get('access_token');if(recoverySession){saveSession({access_token:hashParams.get('access_token'),refresh_token:hashParams.get('refresh_token'),expires_at:Math.floor(Date.now()/1000)+Number(hashParams.get('expires_in')||3600)});history.replaceState(null,'',location.pathname+location.search);}
  ...
  const profileToMember=p=>({id:p.id,name:p.name,email:p.email,phone:p.phone||'',city:p.city||'',role:p.role,organization:p.organization||'',title:p.title||'',status:p.status,verification:p.verification,expires:p.expires,notes:p.notes||'',excludedBenefits:p.excluded_benefit_ids||[],...(p.preferences||{})});
  ...
  window.FoundryBackend={configured,signIn,signOut,restore,bootstrap,sync,uploadImage,issueQr,verifyQr,sendEmail,weather,publicEvent,requestPasswordReset,updatePassword,recoverySession:Boolean(recoverySession),hasSession:()=>Boolean(readSession()),get hydrating(){return hydrating}};
})();
